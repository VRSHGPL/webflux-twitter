...
public class OAuth1SignatureUtil {
    ...
    public OAuth1SignatureUtil(TwitterCredentials credentials) {
    ...
    public String oAuth1Header(ClientRequest request) {
        ...
        OAuthParams.OAuth1Params oAuth1Params = new OAuthParams.OAuth1Params(credentials.getAccessToken(),
                credentials.getConsumerKey(),
                nonce,
                Long.valueOf(timestampSecs),
        ...
        String normalized = this.normalizer.normalize(requestUri.getScheme(), requestUri.getHost(), port,
                request.method().name().toUpperCase(), requestUri.getPath(), requestParams, oAuth1Params);

        String signature;
        try {
            signature = this.signer.getString(normalized, credentials.getAccessTokenSecret(),
